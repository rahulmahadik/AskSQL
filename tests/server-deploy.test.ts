...
import { POSTGRES_DIALECT, type Connector, type CustomModel, type ResultSet, type SchemaCatalog, type TableInfo } from '@asksql/core';
...
function bigCatalog(n: number): SchemaCatalog {
  const tables: TableInfo[] = Array.from({ length: n }, (_, i) => ({
    name: `t_${i}`, kind: 'table' as const,
    columns: [{ name: 'id', dbType: 'bigint', nullable: false }, { name: 'val', dbType: 'text', nullable: true }],
    primaryKey: ['id'], foreignKeys: [], uniques: [], checks: [], indexes: [], source: 'db' as const,
  }));
...
class FakeConnector implements Connector {
  ...
  constructor(readonly id: string, readonly name: string, private readonly tableCount = 1, private readonly down = false) {}
  async connect() { if (this.down) throw new Error('ECONNREFUSED'); }
  async close() {}
  async introspect() { return bigCatalog(this.tableCount); }
...
describe('CORS', () => {
  ...
  beforeAll(async () => {
    ...
    app.use('/asksql', asksqlMiddleware(
      { connectors: [new FakeConnector('c', 'C')], engine: { model }, auth: () => ({ userId: 'alice', allowedConnectionIds: ['c'] }) },
      { cors: true },
    ));
  ...
  it('OPTIONS preflight returns CORS headers + 204', async () => {
    const res = await fetch(`${base}/connections`, { method: 'OPTIONS', headers: { origin: 'https://app.example.com', 'access-control-request-method': 'GET' } });
  ...
  it('actual request reflects the origin', async () => {
    const res = await fetch(`${base}/connections`, { headers: { origin: 'https://app.example.com', 'x-user': 'alice' } });
    expect(res.headers.get('access-control-allow-origin')).toBe('https://app.example.com');
  });
  it('allow-list form rejects an unlisted origin', async () => {
    const app2 = express();
    app2.use(express.json());
    app2.use('/asksql', asksqlMiddleware(
      { connectors: [new FakeConnector('c', 'C')], engine: { model }, auth: () => ({ userId: 'a', allowedConnectionIds: ['c'] }) },
      { cors: ['https://good.com'] },
    ));
...
describe('DB unreachable at startup', () => {
  ...
  beforeAll(async () => {
    ...
    app.use('/asksql', asksqlMiddleware({ connectors: [new FakeConnector('down', 'Down', 1, true)], engine: { model }, auth: () => ({ userId: 'a', allowedConnectionIds: ['down'] }) }));
    ({ server, base } = await listen(app));
  });
  afterAll(() => new Promise<void>((r) => server.close(() => r())));
  ...
  it('a query against the down DB returns a typed error, not a hang', async () => {
    const res = await fetch(`${base}/execute`, { method: 'POST', headers: H, body: JSON.stringify({ sql: 'SELECT 1', connectionId: 'down' }) });
...
describe('audit failure never blocks a query', () => {
  let server: Server; let base = ''; let auditCalls = 0;
  beforeAll(async () => {
    const flakyAudit: AuditSink = { write: async () => { auditCalls++; throw new Error('audit sink down'); } };
    const app = express();
    app.use(express.json());
    app.use('/asksql', asksqlMiddleware({ connectors: [new FakeConnector('c', 'C')], engine: { model }, auth: () => ({ userId: 'a', allowedConnectionIds: ['c'] }), audit: flakyAudit }));
    ({ server, base } = await listen(app));
  });
  afterAll(() => new Promise<void>((r) => server.close(() => r())));

  it('the query still succeeds despite the audit sink throwing', async () => {
    const res = await fetch(`${base}/execute`, { method: 'POST', headers: H, body: JSON.stringify({ sql: 'SELECT 1 AS n FROM t_0', connectionId: 'c' }) });
    expect(res.status).toBe(200);
    const body = await res.json();
...
describe('SSE proxy-safe headers', () => {
  ...
  beforeAll(async () => {
    const app = express();
    app.use(express.json());
    app.use('/asksql', asksqlMiddleware({ connectors: [new FakeConnector('c', 'C')], engine: { model }, auth: () => ({ userId: 'a', allowedConnectionIds: ['c'] }) }));
    ({ server, base } = await listen(app));
  });
  afterAll(() => new Promise<void>((r) => server.close(() => r())));

  it('chat stream sets X-Accel-Buffering:no and event-stream content type', async () => {
    const res = await fetch(`${base}/chat`, { method: 'POST', headers: H, body: JSON.stringify({ question: 'q', connectionId: 'c' }) });
...
describe('large catalog', () => {
  ...
  beforeAll(async () => {
    const app = express();
    app.use(express.json());
    app.use('/asksql', asksqlMiddleware({ connectors: [new FakeConnector('big', 'Big', 5000)], engine: { model }, auth: () => ({ userId: 'a', allowedConnectionIds: ['big'] }) }));
    ({ server, base } = await listen(app));
  });
  afterAll(() => new Promise<void>((r) => server.close(() => r())));
...
describe('server restart mid-session', () => {
  it('in-flight request during a server close surfaces a clean network error, not a hang', async () => {
    ...
    class Hanging extends FakeConnector {
      override async execute(): Promise<ResultSet> { return new Promise(() => {}); }
    }
    app.use('/asksql', asksqlMiddleware({ connectors: [new Hanging('h', 'H')], engine: { model }, auth: () => ({ userId: 'a', allowedConnectionIds: ['h'] }) }));
    const { server, base } = await listen(app);

    const inflight = fetch(`${base}/execute`, { method: 'POST', headers: H, body: JSON.stringify({ sql: 'SELECT 1 AS n FROM t_0', connectionId: 'h' }) });
...
describe('TLS configuration is accepted', () => {
  it('the Postgres connector accepts an ssl option without mangling it', async () => {
    ...
    const c = new PostgresConnector({ id: 't', name: 't', host: '127.0.0.1', port: 5999, user: 'u', password: 'p', database: 'd', ssl: { rejectUnauthorized: false } });
...
describe('special characters in connection config', () => {
  it('a password with @:/#?& is carried as an object field, never string-concatenated', async () => {
    ...
    const { PostgresConnector } = await import('@asksql/postgres');
    const c = new PostgresConnector({ id: 'x', name: 'x', host: 'localhost', user: 'u', password: 'p@ss:w/rd#?&%', database: 'd' });
