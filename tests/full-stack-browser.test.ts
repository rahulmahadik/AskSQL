...
describe('full-stack browser E2E', () => {
  it('type a question -> SQL streams in -> Run -> result table renders', async () => {
    if (!ready || !browser) {
  ...
  it('a category+numeric result renders a chart on toggle', async () => {
  ...
  it('offline mid-session shows a clean error, then recovers online', async () => {
    if (!ready || !browser) return;
    const page = await browser.newPage();
    await page.goto(webBase, { waitUntil: 'networkidle0' });
    await page.waitForSelector('.asksql-input textarea', { timeout: 10_000 });
  ...
  it('navigating away during generation leaves no uncaught error', async () => {
    if (!ready || !browser) return;
    const page = await browser.newPage();
  ...
  it('edit the generated SQL, save, run - the edited query executes', async () => {
    if (!ready || !browser) return;
    const page = await browser.newPage();
    await page.goto(webBase, { waitUntil: 'networkidle0' });
    await page.waitForSelector('.asksql-input textarea', { timeout: 10_000 });
    ...
    await page.keyboard.press('Enter');
    await page.waitForSelector('.asksql-sqlcode', { timeout: 60_000 });
    // Auto-run holds busy (so Stop shows and Edit is disabled mid-run); wait for the result before editing.
    await page.waitForSelector('.asksql-table', { timeout: 60_000 });

    // Click "Edit", replace the SQL with a different valid query, Save.
    const editBtn = await page.$$('.asksql-btn');
    for (const b of editBtn) {
      const t = await b.evaluate((el) => el.textContent);
      if (t && /^edit$/i.test(t.trim())) {
        await b.click();
        break;
      }
    }
    ...
    await page.evaluate(() => {
      (document.querySelector('.asksql-sqledit') as HTMLTextAreaElement).value = '';
    });
