 ...
 */
import { assertBaseUrl, PROVIDER_API_HOST, type ProviderName } from '@asksql/core/providers';
...
async function listOllama(baseURL: string, signal: AbortSignal): Promise<string[]> {
  ...
  return (body.models ?? []).map((m) => m.name).filter((n): n is string => Boolean(n && !isNotChatModel(n)));
}
...
async function listOpenAICompatible(
  ...
  return (body.data ?? []).map((m) => m.id).filter((id): id is string => Boolean(id && !isNotChatModel(id)));
}
