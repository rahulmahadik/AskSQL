...
chrome.runtime.onInstalled.addListener(() => {
  chrome.sidePanel
    ...
    .catch((err: unknown) => console.error('AskSQL: setPanelBehavior failed - clicking the toolbar icon may not open the side panel', err));
  chrome.contextMenus.create(
    ...
    () => {
      if (chrome.runtime.lastError) {
        console.error('AskSQL: contextMenus.create failed - "Ask AskSQL about selection" will be missing', chrome.runtime.lastError);
      }
    },
  );
});
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  ...
  // MUST be called synchronously, before any await, or the user-gesture token
  // backing this click is already spent and the panel refuses to open.
