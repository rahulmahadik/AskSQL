 ...
 * for a change at all. Mirrors core's `grounding.ts`, and is shared by the SQL and MongoDB
 * pipelines so the same prose is judged the same way whichever engine the user is on.
 */
object Grounding {
    ...
    /**
     * SQL an answer quotes as vocabulary, not as a name it claims exists. Kept identical to core's
     * `grounding.ts` list (see tests/kotlin-parity-lists.test.ts); never list a real column name.
     */
    ...
    // Column types and constraint words that read like identifiers but never name a table or
    // column - so a DDL suggestion's `integer`/`unique` isn't mistaken for a proposed object.
     ...
     * something exists in the schema. Only aliases in SQL context count: English
     * "such as customer_history" is exactly the hallucination this floor exists to catch.
     */
    ...
    // `customer_history` and disarmed the floor for exactly the invention it exists to catch.
     ...
     * `explainSchema`. Conservative: only snake_case and quoted/backticked tokens are checked, so
     * ordinary English never trips it while an invented `customer_history` is caught.
     *
     * @param documentStyle MongoDB prose: `$lookup` is an operator, not a collection, and a
     *   double-quoted token is a VALUE, not a quoted identifier as it would be in SQL. Reporting
     *   either as invented made every correct MongoDB answer come back ungrounded.
     */
    fun unknownReferencesInProse(answer: String, catalog: SchemaCatalog, documentStyle: Boolean = false): List<String> {
        ...
        // Document style: a fenced block is a pipeline, and its $lookup spec keys and output
        // aliases are syntax, not claims that a collection exists. SQL keeps its fences in scope.
    ...
    fun mentionsCatalogName(text: String, catalog: SchemaCatalog): Boolean {
        // Whole-word matching: a bare `contains` also fired on "rename"/"namespace" holding "name".
