...
import { createAskSql, POSTGRES_DIALECT, type Connector, type CustomModel, type ResultSet, type SchemaCatalog } from '@asksql/core';
import { createAskSqlMcpTools, buildAskSqlMcpServer } from '../src/index.js';

const CATALOG: SchemaCatalog = {
  engine: 'postgres', schemas: ['public'],
  tables: [{ name: 'users', kind: 'table', columns: [{ name: 'id', dbType: 'bigint', nullable: false }, { name: 'name', dbType: 'text', nullable: true }], primaryKey: ['id'], foreignKeys: [], uniques: [], checks: [], indexes: [], source: 'db' }],
...
class Fake implements Connector {
  ...
  async execute(): Promise<ResultSet> { return { columns: [{ name: 'id', kind: 'bigint' }, { name: 'name', kind: 'text' }], rows: [['1', 'Ada']], rowCount: 1, truncated: false, durationMs: 1, warnings: [] }; }
}
