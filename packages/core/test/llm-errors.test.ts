...
describe('LLM error mapping through the engine', () => {
  ...
  it('ECONNREFUSED -> LLM_UNREACHABLE (names nothing secret)', async () => {
    const err = await askError(throwing(providerErr({ code: 'ECONNREFUSED', message: 'connect ECONNREFUSED 127.0.0.1:11434' })));
...
describe('retry policy actually retries then gives up', () => {
  it('retries a 429 the configured number of times', async () => {
    let calls = 0;
    const model: CustomModel = () => { calls++; return Promise.reject(providerErr({ statusCode: 429 })); };
    await expect(callModel({ model, system: 's', prompt: 'p', settings: { maxRetries: 2, timeoutMs: 5000 } }))
  ...
  it('does NOT retry a 401', async () => {
    let calls = 0;
    const model: CustomModel = () => { calls++; return Promise.reject(providerErr({ statusCode: 401 })); };
    await expect(callModel({ model, system: 's', prompt: 'p', settings: { maxRetries: 3 } }))
...
describe('quota / billing exhaustion is distinct from a transient rate limit', () => {
  ...
  it('tier-limit phrasing ("check your plan limits") is NOT billing', () => {
    const mapped = classifyLlmError(
      quotaErr('Rate limit for your plan tier reached - check your plan limits.'),
      false,
    );
    expect(mapped.code).toBe('LLM_RATE_LIMIT');
    expect(mapped.retryable).toBe(true);
  ...
  it('a 5xx whose body mentions billing stays a retryable outage', () => {
    const mapped = classifyLlmError(
      providerErr({ statusCode: 503, message: 'upstream error', responseBody: '<html>please check your plan and billing details</html>' }),
      false,
    );
  ...
  it('never retries a billing failure at the engine level', async () => {
    ...
    await expect(callModel({ model, system: 's', prompt: 'p', settings: { maxRetries: 3, timeoutMs: 5000 } }))
...
describe('unsupported-temperature detection (reasoning models behind opaque names)', () => {
  it('recognizes the provider 400 for an unsupported temperature parameter', () => {
    expect(isUnsupportedTemperatureError(providerErr({ statusCode: 400, message: "Unsupported parameter: 'temperature' is not supported with this model." }))).toBe(true);
    expect(isUnsupportedTemperatureError(providerErr({ statusCode: 400, message: 'temperature is not supported for reasoning models' }))).toBe(true);
  });

  it('does not fire on unrelated 400s or non-400s', () => {
    expect(isUnsupportedTemperatureError(providerErr({ statusCode: 400, message: 'maximum context length exceeded' }))).toBe(false);
    expect(isUnsupportedTemperatureError(providerErr({ statusCode: 500, message: 'temperature is not supported' }))).toBe(false);
  });
});
