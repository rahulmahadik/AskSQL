...
describe('classifyLlmError', () => {
  ...
  it('ECONNREFUSED -> LLM_UNREACHABLE', () => {
    expect(classifyLlmError(mk({ code: 'ECONNREFUSED', message: 'connect ECONNREFUSED' }), false).code).toBe('LLM_UNREACHABLE');
  });
...
describe('pruneCatalog', () => {
  ...
  it('selects relevant tables + FK closure', () => {
    ...
    many.push(tbl('orders', ['id', 'customer_id'], [{ columns: ['customer_id'], refTable: 'customers', refColumns: ['id'] }]));
...
describe('formatCatalogForPrompt', () => {
  it('renders FK relationships and enum values', () => {
    const c: SchemaCatalog = {
      ...cat([
        { ...tbl('orders', ['id', 'status', 'customer_id'], [{ columns: ['customer_id'], refTable: 'customers', refColumns: ['id'] }]),
          columns: [
