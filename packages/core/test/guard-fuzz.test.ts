...
describe('fuzz: guard never throws, never allows a write', () => {
  ...
  it('SQL-shaped mutations (keywords + punctuation + literals)', () => {
    const token = fc.constantFrom(
      'SELECT', 'select', 'FROM', 'WHERE', 'users', 'orders', '*', ',', ';', '(', ')',
      'DROP', 'DELETE', 'INSERT', 'UPDATE', 'TABLE', 'INTO', 'VALUES', '1', '=', "'x'",
      'UNION', 'ALL', 'JOIN', 'ON', '--', '/*', '*/', 'pg_sleep(1)', 'LIMIT', '100',
      'WITH', 'AS', 'RETURNING', 'OUTFILE', 'FOR', 'UPDATE', 'GRANT', 'COPY',
    );
    fc.assert(
