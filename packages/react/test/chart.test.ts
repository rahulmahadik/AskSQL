...
const rs = (columns: ResultSet['columns'], rows: ResultSet['rows']): ResultSet => ({
  columns, rows, rowCount: rows.length, truncated: false, durationMs: 1, warnings: [],
});

describe('inferChart', () => {
  it('category + numeric -> bar', () => {
    const spec = inferChart(rs(
      [{ name: 'region', kind: 'text' }, { name: 'total', kind: 'number' }],
      [['EU', 100], ['NA', 250], ['APAC', 75]],
    ));
  ...
  it('date + numeric -> line', () => {
    const spec = inferChart(rs(
      [{ name: 'day', kind: 'date' }, { name: 'revenue', kind: 'bigint' }],
  ...
  it('multiple numeric columns -> multi-series', () => {
    const spec = inferChart(rs(
      [{ name: 'region', kind: 'text' }, { name: 'sales', kind: 'number' }, { name: 'refunds', kind: 'number' }],
      [['EU', 100, 5], ['NA', 200, 8]],
    ));
  ...
  it('all-text result is not chartable', () => {
    expect(inferChart(rs([{ name: 'a', kind: 'text' }, { name: 'b', kind: 'text' }], [['x', 'y']]))).toBeNull();
  });
  ...
  it('too many rows is not chartable', () => {
    ...
    expect(inferChart(rs([{ name: 'k', kind: 'text' }, { name: 'v', kind: 'number' }], rows))).toBeNull();
  });

  it('zero rows is not chartable', () => {
    expect(inferChart(rs([{ name: 'k', kind: 'text' }, { name: 'v', kind: 'number' }], []))).toBeNull();
  });
});
